...
LLM
        ↓
공식문서에서 조건 후보 제안 (승인 전 Runtime 사용 금지)
쉬운 설명
번역
다음 행동
은행 문의문
...
> **LLM = 조건 후보 제안·쉬운 설명·번역·문의문·다음 행동**

> **검증기 = LLM 제안을 원문과 대조해 저장 여부 결정**

> LLM은 제안만 하고, 저장과 판정은 결정론적 계층이 담당한다. 인용문이 저장 Snapshot에 그대로 없거나, 원문에 없는 숫자·Visa 코드를 쓰거나, Rule Engine이 비교할 수 없는 값 형식이면 저장하지 않는다. 저장된 후보도 Human Verification 전에는 진단에 사용하지 않는다.
