...
const copy = {
  ko: { eyebrow: "AI-201 ~ AI-204", title: "쉬운 설명과 은행 문의문", description: "숫자와 비자코드는 Rule Engine의 구조화 값을 그대로 표시합니다.", roles: "AI가 필요한 이유", ruleRole: "Rule Engine · 결정론적 판정", ragRole: "RAG · 공식 근거 검색", aiRole: "AI · 쉬운 설명·번역·문의문", facts: "확인에 사용한 값", next: "다음 행동", visa: "체류자격", visaRemaining: "비자 잔여기간", residency: "국내 체류기간", months: "개월", passed: "충족 조건", failed: "미충족 조건", external: "외부 확인", unknown: "비공개 조건", terms: "쉬운 금융용어", inquiry: "은행에 이렇게 문의해 보세요", confirmation: "공식적으로 확인할 항목", korean: "은행 전달용 한국어", translated: "선택 언어 번역", copy: "복사", copied: "복사됨", noInquiry: "현재 결과에는 별도 은행 문의문이 필요하지 않습니다.", error: "AI 설명을 일시적으로 불러오지 못했습니다. 위 Rule Engine 결과는 그대로 유효합니다." },
  en: { eyebrow: "AI-201 ~ AI-204", title: "Plain-language explanation and bank inquiry", description: "Numbers and visa codes are displayed directly from the Rule Engine's structured values.", roles: "Why AI is necessary", ruleRole: "Rule Engine · deterministic decision", ragRole: "RAG · official evidence retrieval", aiRole: "AI · plain explanation, translation, inquiry", facts: "Structured values used", next: "Next actions", visa: "Status of Stay", visaRemaining: "Visa remaining", residency: "Residency in Korea", months: "months", passed: "Conditions met", failed: "Conditions not met", external: "External checks", unknown: "Unpublished conditions", terms: "Financial terms in plain language", inquiry: "Ask the bank with this message", confirmation: "Items to confirm officially", korean: "Korean message for the bank", translated: "Translation in your language", copy: "Copy", copied: "Copied", noInquiry: "This result does not currently require a separate bank inquiry.", error: "The AI explanation is temporarily unavailable. The Rule Engine result above remains valid." },
  vi: { eyebrow: "AI-201 ~ AI-204", title: "Giải thích dễ hiểu và nội dung hỏi ngân hàng", description: "Số liệu và mã visa được hiển thị trực tiếp từ dữ liệu có cấu trúc của Rule Engine.", roles: "Vì sao cần AI", ruleRole: "Rule Engine · phán định xác định", ragRole: "RAG · tìm căn cứ chính thức", aiRole: "AI · giải thích, dịch và tạo câu hỏi", facts: "Giá trị có cấu trúc đã dùng", next: "Hành động tiếp theo", visa: "Tư cách lưu trú", visaRemaining: "Thời hạn visa còn lại", residency: "Thời gian cư trú tại Hàn Quốc", months: "tháng", passed: "Điều kiện đã đạt", failed: "Điều kiện chưa đạt", external: "Kiểm tra bên ngoài", unknown: "Điều kiện không công khai", terms: "Thuật ngữ tài chính dễ hiểu", inquiry: "Hãy hỏi ngân hàng bằng nội dung này", confirmation: "Nội dung cần xác nhận chính thức", korean: "Nội dung tiếng Hàn gửi ngân hàng", translated: "Bản dịch theo ngôn ngữ đã chọn", copy: "Sao chép", copied: "Đã sao chép", noInquiry: "Kết quả hiện tại không cần nội dung hỏi riêng cho ngân hàng.", error: "Tạm thời không thể tải giải thích AI. Kết quả Rule Engine ở trên vẫn giữ nguyên." },
} as const;
...
export function AiExplanationPanel({ data, loading, error }: { data?: AiExplanation; loading: boolean; error: boolean }) {
  ...
  return <section className="mt-6 overflow-hidden rounded-panel border border-line bg-surface" aria-live="polite">
    <header className="border-b border-line bg-surface-subtle p-6 sm:p-8">
      ...
      <section className="mt-5"><h3 className="text-sm font-bold text-ink">{text.roles}</h3><div className="mt-2 grid gap-2 sm:grid-cols-3">{[text.ruleRole, text.ragRole, text.aiRole].map((role) => <p className="rounded-control border border-line bg-surface px-3 py-2 text-xs font-semibold text-muted" key={role}>{role}</p>)}</div></section>
    </header>
