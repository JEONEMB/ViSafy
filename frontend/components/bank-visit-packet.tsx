...
export function BankVisitPacket() {
  ...
  return <Shell>
    ...
    <Section title={text.confirm}>
      {confirmations.length === 0 ? <p className="text-sm text-muted">{text.noConfirm}</p> : <ul className="space-y-2">{confirmations.map((rule) => <li className="flex items-start gap-2 rounded-card border border-line bg-surface-subtle p-3 text-sm" key={`${rule.key}-${rule.messageCode}`}><span className="mt-0.5 inline-block h-4 w-4 shrink-0 rounded-sm border border-ink" aria-hidden /><span className="text-ink">{rule.message}</span></li>)}</ul>}
    </Section>
