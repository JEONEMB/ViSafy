  ...
  return <section className="ui-panel mt-8 scroll-mt-24 p-6 sm:p-8" id="financial-journey" aria-labelledby="financial-journey-heading">
    ...
    {selectedStep ? <section className="mt-7 rounded-panel border border-line bg-surface-subtle p-5 sm:p-6" aria-live="polite">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[0.8fr_1.2fr]">
        ...
        <div><h4 className="font-bold text-ink">{text.products}</h4>{relatedProducts.length ? <div className="mt-3 grid gap-3">{relatedProducts.map((product) => <article className="rounded-card border border-line bg-surface p-4" key={product.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="flex items-center gap-1.5 text-xs font-semibold text-brand"><BankLogo institution={product.institution} size={18} />{institutionLabel(locale, product.institution)}</p><h5 className="mt-1 font-bold text-ink">{productNameLabel(locale, product.productCode, product.productName).name}{productNameLabel(locale, product.productCode, product.productName).original ? <span className="ml-1.5 align-middle text-xs font-medium text-quiet">({productNameLabel(locale, product.productCode, product.productName).original})</span> : null}</h5></div><span className="rounded-full border border-line bg-surface-subtle px-2.5 py-1 text-xs font-semibold text-muted">{journeyAudienceLabel(locale, product.productAudience)}</span></div><p className="mt-3 text-sm leading-6 text-muted">{targetSummaryLabel(locale, product.productCode, product.targetSummary)}</p><dl className="mt-3 grid gap-3 text-xs sm:grid-cols-2"><div className="rounded-control border border-line bg-surface-subtle p-3"><dt className="font-bold text-ink">{text.documents}</dt><dd className="mt-1 leading-5 text-muted">{product.dataPackage.documentEvidence ? productDetailLabel(locale, product.productCode, "requiredDocuments", product.requiredDocuments) : text.unknown}</dd></div><div className="rounded-control border border-line bg-surface-subtle p-3"><dt className="font-bold text-ink">{text.channel}</dt><dd className="mt-1 leading-5 text-muted">{product.dataPackage.channelEvidence ? productDetailLabel(locale, product.productCode, "applicationMethod", product.applicationMethod) : text.unknown}</dd></div></dl><div className="mt-4 flex flex-wrap gap-2"><Link className="ui-button ui-button-secondary min-h-9 px-3 py-1.5 text-xs" href={`/products/${product.id}?tab=documents`}>{text.detail}</Link><a className="ui-button ui-button-primary min-h-9 px-3 py-1.5 text-xs" href={product.sourceUrl} rel="noreferrer" target="_blank">{text.official} ↗</a></div></article>)}</div> : <p className="mt-3 rounded-card border border-dashed border-line-strong p-5 text-sm text-muted">{text.noProducts}</p>}</div>
      </div>
    </section> : null}
  </section>;
}
