...
export function AdminProductEditForm({ product, sources, pending, onCancel, onSave }: {
  ...
  return <form className="mt-5 grid gap-4 rounded-2xl border border-blue-200 bg-blue-50/40 p-5 sm:grid-cols-2" onSubmit={submit}>
    ...
    <label className="text-sm">상품 유형<select className={input} defaultValue={product.productType} name="productType"><option value="CHECKING_ACCOUNT">입출금 계좌</option><option value="SAVINGS">예·적금</option><option value="LOAN">대출</option><option value="CARD">카드</option></select></label>
    <label className="text-sm">금융 목적<select className={input} defaultValue={product.financialPurpose} name="financialPurpose"><option value="ACCOUNT">계좌</option><option value="SAVINGS">예·적금</option><option value="LOAN">대출</option><option value="CARD">카드</option></select></label>
