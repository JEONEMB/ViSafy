...
function DynamicProfileFields({ fields, locale, pending, error, onSubmit }: { fields: DynamicFieldKey[]; locale: keyof typeof dynamicCopy; pending: boolean; error: boolean; onSubmit: (changes: Partial<TempProfileInput>) => void }) {
  ...
  return <form className="mt-5 rounded-panel border border-status-info-border bg-status-info-bg p-6 sm:p-7" onSubmit={submit}>
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-2">{fields.map((field) => <label className="ui-label" key={field}>{text.fields[field]}
      {field === "hasExistingProductAccount" || field === "hasBankAccount" ? <select className="ui-input" defaultValue="" name={field} required><option disabled value="">{text.choose}</option><option value="true">{text.yes}</option><option value="false">{text.no}</option></select> : <input className="ui-input" inputMode={field === "desiredMonthlyAmount" || field === "desiredAmount" ? "numeric" : undefined} min={field === "desiredMonthlyAmount" || field === "desiredAmount" ? 0 : undefined} name={field} required type={field === "desiredMonthlyAmount" || field === "desiredAmount" ? "number" : "text"} />}
    </label>)}</div>
