...
const packetCopy = {
  ko: { title: "창구 준비 패킷", description: "챙겨갈 서류, 창구에서 보여줄 한국어 문장, 확인 요청 항목을 한 장으로 정리합니다.", open: "패킷 열기" },
  en: { title: "Bank visit packet", description: "One page with the documents to bring, the Korean text to show at the counter, and what to ask.", open: "Open packet" },
  vi: { title: "Bộ hồ sơ đến quầy", description: "Một trang gồm giấy tờ cần mang, câu tiếng Hàn để đưa cho nhân viên và nội dung cần hỏi.", open: "Mở bộ hồ sơ" },
  zh: { title: "柜台准备包", description: "一页汇总需携带的材料、在柜台出示的韩语句子以及需确认的事项。", open: "打开准备包" },
  ja: { title: "窓口準備パケット", description: "持参する書類、窓口で見せる韓国語の文章、確認を依頼する項目を1枚にまとめます。", open: "パケットを開く" },
  th: { title: "ชุดเตรียมตัวไปเคาน์เตอร์", description: "รวมเอกสารที่ต้องนำไป ข้อความภาษาเกาหลีที่จะแสดงที่เคาน์เตอร์ และรายการที่ต้องสอบถาม ไว้ในหน้าเดียว", open: "เปิดชุดเตรียมตัว" },
} as const;
const copy = {
...
export function ProductDetailDashboard() {
  ...
  return <main className="ui-page">
    ...
    <header className="ui-card mt-4 p-6 sm:p-8"><div className="flex flex-wrap items-center gap-3"><span className="rounded-full border border-line bg-surface-subtle px-3 py-1 text-xs font-semibold text-muted">{readinessLabels[uiLocale][item.diagnosisStatus]}</span><span className="flex items-center gap-1.5 text-sm font-semibold text-brand"><BankLogo institution={item.institution} size={22} />{institutionLabel(uiLocale, item.institution)}</span></div><h1 className="mt-4 text-3xl font-bold leading-tight text-ink sm:text-4xl">{productNameLabel(uiLocale, item.productCode, item.productName).name}{productNameLabel(uiLocale, item.productCode, item.productName).original ? <span className="ml-2 align-middle text-lg font-medium text-quiet">({productNameLabel(uiLocale, item.productCode, item.productName).original})</span> : null}</h1><p className="mt-4 max-w-reading text-base leading-7 text-muted">{productDetailLabel(uiLocale, item.productCode, "description", item.description)}</p></header>
    <section className="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-panel border border-brand bg-brand-soft p-6"><div className="max-w-reading"><h2 className="text-xl font-bold text-ink">{packetCopy[uiLocale].title}</h2><p className="mt-1 text-sm leading-6 text-muted">{packetCopy[uiLocale].description}</p></div><Link className="ui-button ui-button-primary min-h-12" href={`/products/${id}/packet`}>{packetCopy[uiLocale].open} →</Link></section>
    ...
    {activeTab === "precheck" ? <section className="mt-6" role="tabpanel"><div className="rounded-panel border border-status-info-border bg-status-info-bg p-6 sm:p-8"><div className="flex flex-wrap items-center justify-between gap-5"><div className="max-w-reading"><h2 className="text-2xl font-bold text-ink">{text.run}</h2><p className="mt-2 text-sm leading-6 text-muted">{text.description}</p></div><button className="ui-button ui-button-primary min-h-12" disabled={precheck.isPending || profile.isFetching || profileIdentity === undefined} onClick={run}>{precheck.isPending || profile.isFetching ? text.checking : precheck.data ? text.rerun : text.run}</button></div></div>
      ...
      {precheck.data ? <PrecheckResult locale={uiLocale} result={precheck.data} rules={item.rules} sourceTitle={productDetailLabel(uiLocale, item.productCode, "sourceTitle", item.sourceTitle)} text={text} /> : null}
    ...
    {activeTab === "evidence" ? <section className="mt-6" role="tabpanel"><RuleEvidence rules={item.rules} sourceTitle={productDetailLabel(uiLocale, item.productCode, "sourceTitle", item.sourceTitle)} text={text} /><RagQuestionPanel productId={item.id} rules={item.rules} /></section> : null}
    ...
    {activeTab === "official" ? <section className="mt-6 grid gap-5 sm:grid-cols-2" role="tabpanel"><Info title={text.summary} body={targetSummaryLabel(uiLocale, item.productCode, item.targetSummary)} /><Info title={text.public} body={productDetailLabel(uiLocale, item.productCode, "publicConditions", item.publicConditions)} /><Info title={text.additional} body={productDetailLabel(uiLocale, item.productCode, "additionalConditions", item.additionalConditions)} /><article className="ui-card p-6"><p className="text-xs font-semibold text-brand">{text.source}</p><h2 className="mt-2 font-bold text-ink">{productDetailLabel(uiLocale, item.productCode, "sourceTitle", item.sourceTitle)}</h2><p className="mt-3 text-sm text-muted">{text.base}: {item.informationBaseDate}</p><p className="mt-2 text-xs text-muted">Last reviewed: {new Date(item.sourceTrust.lastVerifiedAt).toLocaleDateString()} · Evidence {item.sourceTrust.evidenceCoveragePercent}%</p><div className="mt-5 flex flex-wrap gap-2"><a className="ui-button ui-button-secondary" href={item.sourceUrl} rel="noreferrer" target="_blank">{text.officialLink} ↗</a>{item.officialApplicationUrl ? <a className="ui-button ui-button-primary" href={item.officialApplicationUrl} rel="noreferrer" target="_blank">Official application ↗</a> : null}</div></article></section> : null}
