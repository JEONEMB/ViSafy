...
export function ProductDetailDashboard() {
  const { locale } = useLocale(); const uiLocale = toLegacyLocale(locale); const text = copy[uiLocale]; const params = useParams<{ id: string }>(); const search = useSearchParams(); const id = Number(params.id);
  ...
  const guidance = useQuery({ queryKey: ["product-guidance", id, locale], queryFn: () => getProductGuidance(id, toLegacyLocale(locale)), enabled: Number.isFinite(id) });
